...
public class StandardDamageDistributionAlgorithm extends DamageDistribution {
    public static final Codec<StandardDamageDistributionAlgorithm> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    Codec.simpleMap(EquipmentSlot.CODEC, StringRepresentable.fromEnum(() -> EnumPlayerPart.VALUES).listOf(), Keyable.forStrings(() -> Arrays.stream(EquipmentSlot.values()).map(EquipmentSlot::getName)))
                            .fieldOf("partMap").forGetter(o -> o.builtList.stream().collect(Collectors.toMap(pair -> pair.getLeft(), pair -> Arrays.asList(pair.getRight())))),
